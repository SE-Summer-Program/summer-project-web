...
public interface ShiftDao extends JpaRepository<Shift,String> {
    ...
    @Query(value = "select shift from Shift shift where shift.lineType=:type and shift.lineName=:line_name")
    List<Shift> queryByLinetypeAndLinename(@Param("type")String type,@Param("line_name")String line_name);
